...
public class Maze {
	...
	public static boolean SolveMaze(int row, int col, int solutionRow, int solutionCol) {
		...
		do {
			...
			// checking if we can visit this cell and to see if the row / col is within the avaliable numbers
			// and EnemyMazeSolution[row][col] == 0 is making sure that the cell is not already visited
			if (row >= 0 && col >= 0 && row < 30 && col < 30 && EnemyMazeSolution[row][col] == 0) {
				...
				// "Did Go: ";
				//if can go a direction and it is the closest direction to the end goal go there
	...
	public static int CanGo(int row, int col) {
		...
		//If the random number corresponds to a direction that is avaliavle return number
		...
		//If it can't go anywhere return escape char
		return 4;
	}
	...
	public static void MazeMaker() {
		...
		do {
			...
			//Make sure the program does not go on a forever loop
	...
	//Prints entire Maze
	public static void PrintMaze(Player player) {
			...
			for (int k = 0; k < 3; k++) {
				for (int j = 0; j < 30; j++) {
					...
					if (k == 0) {
						...
						if (j == ExitRow && i == ExitCol) {
							System.out.print("&");

						} else if (i == player.GetPositionRow() && j == player.GetPositionCol()) {
							System.out.print("$");
						} else {
							System.out.print("0");
						}
	...
	public static void SelectiveMazeDraw(int playerRow, int playerCol, Enemy enemy, Chest chest) {
